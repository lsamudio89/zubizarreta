   ...
   <title><?php echo nombrePagina(basename($_SERVER['PHP_SELF']))." - ".datosSucursal($id_usuario)->nombre_empresa; ?></title>
    ...
    <script type="text/javascript">
		...
		$(document).ready(function () {
			...
			function iconoEliminar(value, row, index) {
				return [
					'<button type="button" onclick="javascript:void(0)" class="btn btn-primary btn-xs eliminar">&nbsp;&nbsp;<span class="glyphicon glyphicon-trash aria-hidden="true"></span>&nbsp;&nbsp;&nbsp;</button>'
				].join('');
			}
			...
			window.eliminarFila = {
				'click .eliminar': function (e, value, row, index) {
					$("#mensaje").html("");
					var conf = confirm("¿Marcar producto: \""+row.producto+"\" como extraviado?");
					...
					if (conf){
						$.ajax({
							...
							async: true,
							type: 'POST',
							url: 'inc/productos-transito-data.php',
							cache: false,
							data: {q: 'extraviado', id_compra_detalle: row.id_compra_detalle },	
							beforeSend: function(){
							...
							success: function (data, status, xhr) {
								...
								var n = data.toLowerCase().indexOf("error");
									if (n == -1) {
										$('#tabla').bootstrapTable('refresh', {url: 'inc/productos-transito-data.php?q=ver&estado='+$("#estado").val()});
									}
							},
							error: function (xhr) {
			...
			$table.bootstrapTable({
				...
				columns: [
					[
						...
						{	field: 'flete', align: 'right', valign: 'middle', title: 'Flete', sortable: true, editable: true	},
						{	field: 'despacho', align: 'right', valign: 'middle', title: 'Despacho', sortable: true, editable: true	},
						{	field: 'costo', align: 'right', valign: 'middle', title: 'Costo', sortable: true, editable: true	},
						{	field: 'total', align: 'right', valign: 'middle', title: 'Total', sortable: true	},
						{	field: 'fecha', align: 'center', valign: 'middle', title: 'Fecha Compra', sortable: true	},
						{	field: 'estado', align: 'center', valign: 'middle', title: 'Estado', sortable: true	}, 
						{	field: 'eliminar', align: 'center', valign: 'middle', title: 'Extraviado', sortable: false, events: eliminarFila,  formatter: iconoEliminar	}
					]
				]
			});
			...
			$table.on('editable-save.bs.table', function (e, field, row, old, $el) {
				...
				/*if (field=="flete" || field=="despacho"){
					var valor = quitaSeparadorMiles(row[field]);
					if (field=="flete") valor = valor + quitaSeparadorMiles(row['despacho']);
					if (field=="despacho") valor = valor + quitaSeparadorMiles(row['flete']);
					$table.bootstrapTable('updateCell', {index:indice, field:"costo", value: valor});
				}*/
				//$table.bootstrapTable('resetView', {});
				
				//guarda el flete y costo al editar
				if (field=="flete" || field=="despacho" || field=="costo"){
					var id = row['id_compra_detalle'];
					var flete = quitaSeparadorMiles(row['flete']);
					var despacho = quitaSeparadorMiles(row['despacho']);
					var costo = quitaSeparadorMiles(row['costo']);
					$.post("inc/productos-transito-data.php",
					{
						q:"guardar-flete",
						flete:flete,
						costo:costo,
			...
			$('#estado').change(function(){
				$("#mensaje").html("");
				$('#tabla').bootstrapTable('refresh', {url: 'inc/productos-transito-data.php?q=ver&estado='+$(this).val()});
			});
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<table id="tabla" data-url="inc/productos-transito-data.php?q=ver&estado=En tránsito" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left"
